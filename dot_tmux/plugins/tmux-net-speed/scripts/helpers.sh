...
get_interfaces()
{
    ...
    if [[ -z "$interfaces" ]] ; then
        if is_osx; then
            # macOS: default to en0 (Wi-Fi), en4-en6 (Ethernet), en8 (iPhone USB)
            interfaces="en0 en4 en5 en6 en8"
        else
...
sum_speed()
{
    ...
    if is_osx; then
        ...
        # Column 7 = Ibytes (download), Column 10 = Obytes (upload)
        local osx_col
        if [[ $column -eq 1 ]]; then
            osx_col=7   # Ibytes for download
        else
            osx_col=10  # Obytes for upload
        fi
        for intf in ${interfaces[@]} ; do
            # Get bytes for this interface (skip header, get numeric value)
            speed=$(netstat -ib 2>/dev/null | awk -v intf="^${intf}$" -v col=$osx_col '$1 ~ intf && $3 ~ /Link/ {print $col}' | head -1)
            let val+=${speed:=0}
        done
    else
